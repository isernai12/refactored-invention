...
def submit_deposit(amount):
    ...
    if screenshot and screenshot.filename != '':
        filename = secure_filename(f"deposit_{g.user['id']}_{screenshot.filename}")
...
def redeem():
    ...
    if form.validate_on_submit():
        ...
        if not redeem_code:
            error = 'ভুল কোড।'
        elif redeem_code['is_used'] == 1:
            error = 'এই কোডটি ইতিমধ্যে ব্যবহৃত হয়েছে।'
        elif redeem_code['expires_at'] and datetime.now() > redeem_code['expires_at']:
            error = 'এই কোডটির মেয়াদ শেষ হয়ে গেছে।'
